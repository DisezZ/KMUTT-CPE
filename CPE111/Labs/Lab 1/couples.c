...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (strcasecmp(people[findStatus[0]]->gender, people[findStatus[1]]->gender) == 0)
        {
            printf("\tError - Couple can't be the same gender\n");
            continue;
        }

        if (strcasecmp(nameInput[0], people[findStatus[0]]->name) == 0)
        {
            ...
            people[findStatus[0]]->pPartner = people[findStatus[1]];
        }
        if (strcasecmp(nameInput[1], people[findStatus[1]]->name) == 0)
        {
            ...
            people[findStatus[1]]->pPartner = people[findStatus[0]];
        }
    }
    //printf("Exit\n");
    printf("\nCouples:\n");
    for (int i = 0; i < count; i++)
    {
        for (int j = 0; j < count; j++)
        {
            if (people[i]->pPartner != NULL && people[j]->pPartner != NULL)
            {
                if (strcasecmp(people[i]->pPartner->name, people[j]->name) == 0 && i < j)
                {
