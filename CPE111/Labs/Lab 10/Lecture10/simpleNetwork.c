...
{
  VERTEX_T *currentVtx = NULL; /* to hold current vetex that are processing */
  int currentIndex;            /* to hold index of current vertex that are processing */
  int i;                       /* for iterating */
  if (vertices[index2].parentIndex != -1)
  {
    currentIndex = index2;
    do
    {
      enqueueMin(&vertices[currentIndex]);
      currentIndex = vertices[currentIndex].parentIndex;
    } while (currentIndex != index1);
    printf("Min total weight of shortest path: %d\n", vertices[index2].dValue);
    printf("Path:\n");
    printf("'%d' ", vertices[index1].keyValue);
    while (queueMinSize() != 0)
    {
      currentVtx = dequeueMin();
      printf("==> '%d' ", currentVtx->keyValue);
    }
    printf("\n");
  }
  else
...
void printShortestPath(int index1, int index2)
{
  ...
  int i;                        /* for iterating */
  int currentIndex;             /* to hold index of current vertex that are processing */
  int weight;                   /* to hold weight of the edge */
  ...
  while (queueMinSize() != 0)
  {
    ...
    for (i = 0; i < MAXVERT; i++)
    {
      if (edges[currentIndex][i] != 0 && vertices[i].color == WHITE)
      {
        if (currentVtx->dValue + edges[currentIndex][i] < vertices[i].dValue)
        {
          vertices[i].dValue = currentVtx->dValue + edges[currentIndex][i];
