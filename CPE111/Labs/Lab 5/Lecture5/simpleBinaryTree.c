 ...
 * 
 * Modified By Lutfee Deemae ID:63070503448
 * Modified date 21 February 2021
 */
...
int calculateMaxDepth(TREE_T *pTree)
{
    ...
    NODE_T *pTempt2 = NULL;
    int size = 0;
    size = maxDepth(pTree->root);
}

int maxDepth(NODE_T *pNode)
{
    if (pNode != NULL)
    {
        if (maxDepth(pNode->left) > maxDepth(pNode->right))
        {
            return maxDepth(pNode->left) + 1;
        }
        else
        {
            return maxDepth(pNode->right) + 1;
        }
    }
    else
    {
...
{
    NODE_T *pTempt = NULL; /* pointer to the current visit node */
    queueClear();
    if (pTree->root != NULL)
    {
        enqueue(pTree->root);
